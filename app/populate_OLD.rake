...
#    end
    ...
    Referrallog.populate 6 do |referrallog|
      Referral.populate 2 do |referral|
        referral.name = Faker::Name.name
        referrallog.referral_id = referral.id
        referrallog.count = 1..20
        referrallog.date = Time.now
        Facility.populate 1 do |facility|
          referrallog.facility_id = facility.id
          facility.name = Faker::Company.name
          facility.address = Faker::Address.street_address
        end
      end
...
#    end
    ...
    Eventlog.populate 10 do |eventlog|
      ...
      Visitor.populate 4 do |visitor|
        visitor.fname = Faker::Name.first_name
        eventlog.visitor_id = visitor.id
      end
    end
